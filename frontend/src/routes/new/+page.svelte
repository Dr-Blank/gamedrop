<script>
	...
	async function load(reset = true) {
		...
		try {
			const res = await feedNew(page, LIMIT);
...
<div class="space-y-5">
	<div>
		<h1 class="flex items-center gap-2 text-2xl font-bold tracking-tight">
			<Sparkles class="size-6 text-primary" /> New additions
		</h1>
		<p class="text-sm text-muted-foreground">Recently discovered games, freshest first.</p>
	</div>
	...
	{:else if items.length === 0}
		<div class="flex flex-col items-center gap-2 rounded-xl border border-dashed py-16 text-center">
			<Sparkles class="size-10 text-muted-foreground/40" />
			<p class="font-medium">Nothing tracked yet</p>
		</div>
	{:else}
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
